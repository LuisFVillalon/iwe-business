...
import { packagesArr, servicesArr, subscriptionsArr } from "@/app/lib/servicesData";
          ...
          <div>
            ...
            >
              <span className="font-semibold flex items-center">
                ...
                Pages Included ({pkg.pages.length})
              </span>
              <ChevronDown 
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <section className="12">
          <div className="text-center">
            <h2 id="packages" className="text-[#0A0A23] text-4xl font-bold text-center mb-2 drop-shadow-lg">Website Packages Tailored for Small Businesses</h2>
            <p className="text-lg text-[#23508e] max-w-3xl mx-auto leading-relaxed mb-8">
              From independent freelancers to thriving businesses, we create websites designed to support your growth and success.
            </p>            
          </div>
          ...
          {packagesArr.length > 0 && (
            <div className="mb-12 mt-8">
              <h3 className="text-center text-2xl font-bold my-4 text-[#0A0A23]  drop-shadow-lg">{packagesArr[0].title}</h3>
              <div className=" bg-white rounded-xl shadow-xl p-8 border border-gray-200 ransition-all duration-300 backdrop-blur-sm bg-opacity-95">
                <div className="grid md:grid-cols-1 gap-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {packagesArr[0].features?.map((feature, index) => (
                        <Feature key={index} {...feature} />
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Basic SEO Section */}
                  {packagesArr[0].seo && packagesArr[0].seo.length > 0 && (
                    <div>
                      <h4 className="text-xl font-semibold mb-4 flex items-center" style={{ color: '#0A0A23' }}>
                        ...
                        {packagesArr[0].seo[0].feature || packagesArr[0].seo[0].title}
                      </h4>
                      <p className="text-gray-600 mb-6 leading-relaxed">
                        {packagesArr[0].seo[0].description}
                      </p>
                      <div className="space-y-3">
                        {packagesArr[0].seo.slice(1).map((seoItem, index) => (
        ...
        <section className="mb-12">
          <h2 id="services" className="text-4xl font-bold text-center mb-6 text-[#0A0A23] drop-shadow-lg">Ongoing Services</h2>
        ...
        {/* Subscriptions */}
        <section className="mb-12">
          <h2 id="subscriptions" className="text-4xl font-bold text-center mb-6 text-[#0A0A23] drop-shadow-lg">Add-On Subscriptions</h2>
          <div className="grid gap-8 md:grid-cols-2">
            {subscriptionsArr.map((subscription, index) => (
              <ServiceSection
                key={index}
